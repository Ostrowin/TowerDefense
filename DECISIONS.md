...
**Status:** zrealizowana (Etapy 0–3, 2026-09-25) z poprawkami z D25/D26: 12 dowódców (4 rasy), boss rywala usunięty. Podkop = efekt dla całej armii; gibony = kopia goryli z WebSlashera.
...
### D26 — Dowódcy w grze: wyjątek od D4, grywalność z danych, bot i kontrakt regresji (2026-09-25)
**Decyzja:** dowódca to jedyna sterowana postać (**wyjątek od D4** — armia dalej walczy sama): `Sim.Hero extends Unit` w `units` i siatce celów, więc wieże, pociski i obszar widzą go bez przeróbek; nie liczy się do limitów, armii ani obrony ścieżek; ginie i odradza się przy bazie (20 s + 2 s/falę, maks. 40 s, 2 s nietykalności). Sterowanie jak w Kingdom Rush: stuknij dowódcę albo portret, stuknij teren; po rozkazie zostaje zaznaczony, stuknięcie w budynek/złoże działa normalnie i odznacza. Umiejętności (3 dowódcy + 1 rasy) to dane z 17 typów efektów, każdy testowany dla obu drużyn; umiejętności dowódcy mają zasięg od niego i czekają, gdy nie żyje, rasowa działa zawsze. **Dowódca jest grywalny, gdy Sim obsługuje typy wszystkich jego umiejętności** (`Cfg.commander_ready`) — menu pokazuje resztę jako „Wkrótce”, a „Weteran” (dawne Deszcz strzał/Pobór/Naprawa) zastępuje rasę bez grywalnego dowódcy. Bot `bot_test` gra dowódcą według R9: w natarciu idzie za czołem armii, przy HP < 30% wraca, rzuca po typie efektu.
**Dlaczego:** grywalność z danych usunęła ręczne flagi — po T9 okazało się, że Snajper i Magma działają razem z Saperem; po grze na telefonie użytkownik zostawił ich grywalnych. Bot stojący przy linii zbiórki nie wpływał na wynik (tabela nie odróżniała dowódców) — dopiero dowódca przy czole armii dał mierzalne różnice. Kontrakt regresji (tabela botów bez dowódcy = 8d8fa4c, `tests/bot_baseline.txt`) pilnował, że ogólna walka nie zmieniła się przy dokładaniu bohatera, wzmocnień i nowych typów.
